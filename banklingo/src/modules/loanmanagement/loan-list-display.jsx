...
class LoanListDisplay extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      this.props.loanState !== prevProps.loanState ||
      this.props.loanSubState !== prevProps.loanSubState
    ) {
  ...
  retrieveFromApi = (tempData) => {
    ...
    let params = `FullDetails=${FullDetails}&SearchText=${SearchText}&PageSize=${PageSize}&CurrentPage=${CurrentPage}&StartDate=${startDate}&endDate=${endDate}&LoanState=${
      this.props.loanState
    }&LoanSubState=${this.props.loanSubState ?? LoanSubStateConstants.All}`;
  ...
  getHeaderDescription = () => {
    ...
    switch (this.props.loanState) {
      case LoanStateConstants.ALL_LOANS:
        return <div>Loan Accounts (All)</div>;
      case LoanStateConstants.PENDING_APPROVAL:
        return <div>Loan Accounts (Pending Approval)</div>;
      case LoanStateConstants.APPROVED:
        return <div>Loan Accounts (Approved)</div>;
      case LoanStateConstants.REJECTED:
        return <div>Loan Accounts (Rejected)</div>;

      case LoanStateConstants.ACTIVE:
        return <div>Loan Accounts (Active)</div>;
      case LoanStateConstants.IN_ARREARS:
        return <div>Loan Accounts (Active -In Arrears)</div>;
      case LoanStateConstants.CLOSED:
        return <div>Loan Accounts (Closed - All Obligations Met)</div>;
      case LoanStateConstants.CLOSED_WITHDRAWN:
        return <div>Loan Accounts (Closed - Withdrawn)</div>;
      case LoanStateConstants.CLOSED_WRITTEN_OFF:
        return <div>Loan Accounts (Closed - Wwritten Off)</div>;
      default:
        return <div>Loan Accounts (All)</div>;
    }
  };
  ...
  searchTxtn = (e, tempData) => {
    e.preventDefault();
    //const { dispatch } = this.props;
    let { SearchText, endDate, startDate } = this.state;
    ...
    if (SearchText !== "" || endDate !== "" || startDate !== "") {
      ...
      this.setState(
        { SearchText: SearchText, endDate: endDate, startDate: startDate },
        () => {
          this.retrieveFromApi();
        }
      );
        ...
        <InnerPageContainer {...this.props}>
          <div className="content-wrapper">
            <div className="module-heading">
              ...
              <SubMenu
                links={LOAN_MODULE_MENU_LINKS}
                key={this.props.loanState}
              />
