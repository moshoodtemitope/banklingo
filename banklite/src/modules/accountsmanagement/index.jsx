...
class AccountManagement extends React.Component {
  ...
  exportGLAccounts = () => {
    ...
    dispatch(acoountingActions.exportGLAccounts(payload));
  };
  ...
  handleEditGlShow = (glIdToEdit, accountToEdit) =>
    this.setState({ showEditGL: true, glIdToEdit, accountToEdit });

  filterWithType = (typeToShow, filterType) => {
    let getGLAccountsRequest = this.props.getGLAccounts;

    let allAccountsData =
      this.props.getGLAccounts.request_data !== undefined &&
      this.props.getGLAccounts.request_data.response !== undefined
        ? this.props.getGLAccounts.request_data.response.data
        : null;
  ...
  returnGLTypeCount = (glType, glAccountsStats, isAll) => {
    ...
    if (isAll === undefined || isAll === null) {
      if (glAccountsStats.length >= 0) {
        filteredStat = glAccountsStats.filter(
          (eachStat) => eachStat.accountTypeDesctiption === glType
        );
  ...
  renderAllGL = () => {
    ...
    let saveRequestData =
      getGLAccountsRequest.request_data !== undefined
        ? getGLAccountsRequest.request_data.tempData
        : null;
    ...
    switch (getGLAccountsRequest.request_status) {
        ...
        if (saveRequestData === null || saveRequestData === undefined) {
          return (
            <div className="loading-content">
              {createGLAccountRequest.request_status ===
                accountingConstants.CREATE_GLACCOUNTS_SUCCESS && (
          ...
          let getGLAccountsData =
            ...
            getGLAccountsStatsData =
              saveRequestData.result2 !== undefined
                ? saveRequestData.result2
                : this.statsTemp;
          ...
          if (unfilteredAccounTypes instanceof Array) {
            ...
            Object.keys(countOfAccounTypes).forEach(function (v) {
              countOfAccounTypes[v] = {
                index: countOfAccounTypes[v],
                length: countOfAccounTypes[v].length,
              };
            });
          }
          ...
          return (
            <div>
              ...
              {updateGLAccount.request_status ===
                accountingConstants.UPDATE_GLACCOUNTS_SUCCESS && (
                <Alert variant="success">
              ...
              <div className="filter-nav">
                {/* <div className="filter-nav"> */}
                <div
                  className={typeToShow === "all" ? "active-type" : ""}
                  onClick={(e) => this.filterWithType("all", 0)}
                >
                  All(
                  {this.returnGLTypeCount("All", getGLAccountsStatsData, "all")}
                  )
                </div>
                <div
                  className={typeToShow === "assets" ? "active-type" : ""}
                  onClick={(e) => this.filterWithType("assets", 1)}
                >
                  Asset(
                  {this.returnGLTypeCount("Asset", getGLAccountsStatsData)})
                </div>
                <div
                  className={typeToShow === "liability" ? "active-type" : ""}
                  onClick={(e) => this.filterWithType("liability", 2)}
                >
                  Liability(
                  {this.returnGLTypeCount("Liability", getGLAccountsStatsData)})
                </div>
                <div
                  className={typeToShow === "equity" ? "active-type" : ""}
                  onClick={(e) => this.filterWithType("equity", 3)}
                >
                  Equity(
                  {this.returnGLTypeCount("Equity", getGLAccountsStatsData)})
                </div>
                <div
                  className={typeToShow === "income" ? "active-type" : ""}
                  onClick={(e) => this.filterWithType("income", 4)}
                >
                  Income(
                  {this.returnGLTypeCount("Income", getGLAccountsStatsData)})
                </div>
                <div
                  className={typeToShow === "expense" ? "active-type" : ""}
                  onClick={(e) => this.filterWithType("expense", 5)}
                >
                  Expense(
                  {this.returnGLTypeCount("Expense", getGLAccountsStatsData)})
                </div>
                    ...
                    {getGLAccountsData.map((eachGL, index) => {
                      return (
                        <tr
                          key={index}
                          className={
                            eachGL.accountUsageId === 1 ? "heading-row" : ""
                          }
                        >
                          <td>{eachGL.glCode}</td>
                          ...
                          {eachGL.accountUsageId !== 1 && (
                            <td>
                              {eachGL.manualEntriesAllowed.toString() === "true"
                                ? "Allowed"
                                : "Not Allowed"}
                            </td>
                          )}
      ...
      case accountingConstants.GET_GLACCOUNTS_SUCCESS:
        let getGLAccountsData =
            getGLAccountsRequest.request_data.response.data.result,
          getGLAccountsStatsData =
            getGLAccountsRequest.request_data.response.data.result2,
          accounTypes = [],
                  ...
                  <div className="pagination-wrap">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        this.setPagesize(
                          e,
                          getGLAccountsRequest.request_data.response.data
                        )
                      }
                      value={this.state.PageSize}
                      className="countdropdown form-control form-control-sm"
                          ...
                          <td>
                            ...
                            >
                              <Dropdown.Item
                                eventKey="1"
                                onClick={() =>
                                  this.handleEditGlShow(
                                    eachGL.glCode,
                                    eachGL.accountTypeDescription
                                  )
                                }
                              >
                                Edit
  ...
  newGLAccountPopUp = () => {
    let allAccountTypes = [
      ...
      glAccountValidationSchema = Yup.object().shape({
        accountNotes: Yup.string().min(1, "Min of two characters"),
        ...
        <Modal.Body>
          <Formik
            ...
            onSubmit={(values, { resetForm }) => {
              ...
              this.submitNewGLAccountDetails(newgLPayload).then(() => {
                if (
                  this.props.createGLAccount.request_status ===
                  accountingConstants.CREATE_GLACCOUNTS_SUCCESS
                ) {
                  resetForm();
                  this.handleClose();
                  ...
                  setTimeout(() => {
                    this.getGLAccounts(allAccountsData);
                    this.props.dispatch(
                      acoountingActions.createGLAccount("CLEAR")
                    );
                  }, 2000);
                }
                // else{
              ...
              <Form noValidate onSubmit={handleSubmit}>
                <Form.Row>
                  <Col>
                    ...
                    <Form.Control
                      ...
                      className={
                        errors.glCode && touched.glCode ? "is-invalid" : null
                      }
                      required
                    />
                ...
                <Form.Group className="table-helper">
                  <input
                    ...
                    onChange={handleChange}
                    checked={
                      values.allowManualEntry ? values.allowManualEntry : null
                    }
                    value={values.allowManualEntry}
                    id="allowManualEntry"
                  />
                  <label htmlFor="allowManualEntry">
                ...
                <div className="footer-with-cta toleft">
                  ...
                  >
                    {createGLAccountRequest.is_request_processing
                      ? "Please wait..."
                      : "Save"}
                  </Button>
                </div>
          ...
          {createGLAccountRequest.request_status ===
            accountingConstants.CREATE_GLACCOUNTS_FAILURE && (
  ...
  editGLAccountPopUp = () => {
    let allAccountTypes = [
      ...
      glAccountValidationSchema = Yup.object().shape({
        accountNotes: Yup.string().min(1, "Min of two characters"),
        accountUsage: Yup.string().min(1, "Please provide valid response"),
        accountType: Yup.string().min(1, "Min of two characters"),
        accountName: Yup.string()
          .min(2, "Min of two characters")
          .max(30, "Max Limit reached")
          .required("Please provide name"),
    ...
    selectGlAcc = gLAccountsList.filter(
      (account, index) => account.glCode === glIdToEdit
    )[0];
      ...
      return (
        ...
        >
          ...
          <Modal.Body>
            <Formik
              ...
              onSubmit={(values, { resetForm }) => {
                let updatedGlPayload = {
                  ...
                  accountName: values.accountName,
                  accountTypeId:
                    selectedAccType != undefined
                      ? parseInt(selectedAccType.value)
                      : selectGlAcc.accountTypeId,
                  accountUsageId:
                    selectedUsageOption != undefined
                      ? parseInt(selectedUsageOption.value)
                      : selectGlAcc.accountUsageId,
                  manualEntriesAllowed: values.allowManualEntry,
                  notes: values.accountNotes,
                  idToUpdate: selectGlAcc.id,
                };

                this.submitUpdatedGLAccountDetails(updatedGlPayload).then(
                  () => {
                    if (
                      this.props.updateGLAccount.request_status ===
                      accountingConstants.UPDATE_GLACCOUNTS_SUCCESS
                    ) {
                      // resetForm();
                      ...
                      setTimeout(() => {
                        this.getGLAccounts(allAccountsData);
                        this.props.dispatch(
                          acoountingActions.updateGLAccount("CLEAR")
                        );
                      }, 2000);
                    } else {
                      setTimeout(() => {
                ...
                <Form noValidate onSubmit={handleSubmit}>
                    ...
                    <Col>
                      <Form.Group controlId="countriesList">
                        ...
                        <Select
                          ...
                          defaultValue={{
                            label: selectGlAcc.accountTypeDescription,
                            value: selectGlAcc.accountTypeId,
                          }}
                          name="accountType"
                          required
                        />
                    ...
                    <Col>
                      <Form.Group>
                        ...
                        <Select
                          ...
                          defaultValue={{
                            label: selectGlAcc.accountUsageDescription,
                            value: selectGlAcc.accountUsageId,
                          }}
                          name="accountUsage"
                          required
                        />
                  ...
                  <div className="footer-with-cta toleft">
                    <Button
                      variant="secondary"
                      className="grayed-out"
                      onClick={this.handleEditGlClose}
                    >
                      Cancel
                    </Button>
                    <Button
                    ...
                    >
                      {updateGLAccount.is_request_processing
                        ? "Please wait..."
                        : "Update"}
                    </Button>
                  </div>
            ...
            {updateGLAccount.request_status ===
              accountingConstants.UPDATE_GLACCOUNTS_FAILURE && (
              <Alert variant="danger">
    ...
    return (
      <Fragment>
        <InnerPageContainer {...this.props}>
          ...
          {this.props.getGLAccounts.request_status ===
            accountingConstants.GET_GLACCOUNTS_SUCCESS &&
            ...
            <div className="module-heading">
              ...
              <div className="module-content">
                <div className="content-container">
                  <div className="row">
                    <div className="col-sm-12">
                      <div className="middle-content">{this.renderAllGL()}</div>
                    </div>
                  </div>
                </div>
              </div>
