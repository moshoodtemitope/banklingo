...
class ManageCustomerWhitelist extends React.Component {
    ...
    handleCloseNewRecord = () => {
        if(this.props.createCreditScoreByPassReducer.is_request_processing===false){
    ...
    handleEditRecordClose = () => {
        if (this.props.updateCreditScoreByPassReducer.is_request_processing === false) {
    ...
    updateRecordPopUp = (updateType) => {
        ...
        let updateCreditScoreByPassRequest = this.props.updateCreditScoreByPassReducer;
        ...
        let checkValidationSchema = Yup.object().shape({
            creditScore:  Yup.string()
                .required('Required'),
            loanCreditScoreByPassStatus:  Yup.string()
                .required('Required'),
            amount:  Yup.string()
                .required('Required'),
            bvn:  Yup.string()
                ...
                <Modal.Body>
                    ...
                    {updateCreditScoreByPassRequest.request_status === platformConstants.UPDATE_CREDIT_SCORE_BYPASS_SUCCESS &&
                        <Alert variant="success">
                            {updateCreditScoreByPassRequest.request_data.response.data.message}
                        </Alert>
                    }
                    {updateCreditScoreByPassRequest.request_status === platformConstants.UPDATE_CREDIT_SCORE_BYPASS_FAILURE &&
    ...
    createNewRecordPopUp = () =>{
        ...
        let checkValidationSchema = Yup.object().shape({
                ...
                amount:  Yup.string()
                    .required('Required'),
                whitelistOption:  Yup.string()
                    .required('Required'),
                bvn:  Yup.string()
                    .when('whitelistOption',{
                        is:(value)=>value==="bvn",
                        then: Yup.string()
                            .required('Required')
                    }),
                    ...
                    <Formik
                        ...
                        onSubmit={(values, { resetForm }) => {
                            ...
                            this.createNewRecord(requestPayload)
                                .then(
                                    () => {
                                        if(this.props.createCreditScoreByPassReducer.request_status === platformConstants.CREATE_CREDIT_SCORE_BYPASS_SUCCESS){
                                            resetForm();
                                            this.loadInitialData();
                                        }
                            ...
                            errors, }) =>{ 
                                ...
                                return(
                                    <Form noValidate
                                        ...
                                        <Form.Row>
                                            <Col>
                                                <Form.Label className="block-level">Whitelist Option</Form.Label>
                                                <select id="toshow"
                                                    onChange={handleChange}
                                                    value={values.whitelistOption}
                                                    name="whitelistOption"
                                                    className={errors.whitelistOption && touched.whitelistOption ? "is-invalid form-control" : "form-control"}
                                                >
                                                    <option value="">Select</option>
                                                ...
                                                {errors.whitelistOption && touched.whitelistOption ? (
                                                        <span className="invalid-feedback">{errors.whitelistOption}</span>
                                                ) : null}
                                            </Col>
                                        </Form.Row>
                                        ...
                                        <Form.Row>
                                            <Col>
                                                ...
                                                <select id="toshow"
                                                    ...
                                                    onChange={handleChange}
                                                    value={values.loanCreditScoreByPassStatus}
                                                    className={errors.loanCreditScoreByPassStatus && touched.loanCreditScoreByPassStatus ? "form-control form-control-sm is-invalid h-38px" : "form-control form-control-sm h-38px"}
                                                >
                                                    <option value="">Select</option>
                                        ...
                                        {(values.loanCreditScoreByPassStatus ==="1" && values.whitelistOption!=="") &&
                                            <Form.Row>
                                                <Col>
                                                    <Form.Label className="block-level">Amount (₦)</Form.Label>
                                                    <Form.Control type="text"
                                        ...
                                        <div className="footer-with-cta toleft">
                                            <Button
                                                disabled={createCreditScoreByPassRequest.is_request_processing}
                                                variant="secondary" className="grayed-out" onClick={this.handleCloseNewRecord}>Cancel</Button>
                                            <Button
                                                type="submit"
                                                disabled={createCreditScoreByPassRequest.is_request_processing}>
                                                {createCreditScoreByPassRequest.is_request_processing ? "Please wait..." : "Create"}
                                            </Button>
                                        </div>
                                    </Form>
                            )}}
    ...
    renderContentWrap =()=>{
        let fetchAllCreditScoreByPassRequest = this.props.fetchAllCreditScoreByPassReducer;

        let saveRequestData= fetchAllCreditScoreByPassRequest.request_data!==undefined?fetchAllCreditScoreByPassRequest.request_data.tempData:null;
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                ...
                                                                >
                                                                    ...
                                                                    { eachItem.loanCreditScoreByPassStatus===0 &&  <Dropdown.Item eventKey="1">Whitelist</Dropdown.Item>}
                                                                    { eachItem.loanCreditScoreByPassStatus===1 &&  <Dropdown.Item eventKey="1">Blacklist</Dropdown.Item>}
                                                                </DropdownButton>
                                                            </td>
                                                        </tr>
                                                    </Fragment>
...
function mapStateToProps(state) {
    return {
        fetchAllCreditScoreByPassReducer : state.platformReducers.fetchAllCreditScoreByPassReducer,
        fetchSingleCreditScoreByPassReducer : state.platformReducers.fetchSingleCreditScoreByPassReducer,
        createCreditScoreByPassReducer : state.platformReducers.createCreditScoreByPassReducer,
        updateCreditScoreByPassReducer : state.platformReducers.updateCreditScoreByPassReducer,
