...
class CustomerTypesAdministration extends React.Component {
  ...
  initializeState = () => {
    this.state = {
      ...
      PageSize: 25,
      CurrentPage: 1,
      TypeConstant: this.props.TypeConstant,
    };
    return this.state;
  };
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      this.props.TypeConstant !== prevProps.TypeConstant ||
      this.props.TypeConstant !== prevProps.TypeConstant
    ) {
  ...
  retrieveFromApi = (tempData) => {
    const { dispatch } = this.props;
    let { PageSize, FullDetails, CurrentPage, TypeConstant } = this.state;
    let params = `ClientClassification=${TypeConstant}&PageSize=${PageSize}&CurrentPage=${CurrentPage}&CurrentSelectedPage=${CurrentPage}`;
    if (tempData) {
      dispatch(
        customerTypeActions.getCustomerTypes(TypeConstant, params, tempData)
      );
    } else {
      dispatch(customerTypeActions.getCustomerTypes(TypeConstant, params));
    }
  };
  ...
  retrieveFromApi = (tempData) => {
    const { dispatch } = this.props;
    let { PageSize, CurrentPage, TypeConstant } = this.state;
    let params = `ClientClassification=${TypeConstant}&PageSize=${PageSize}&CurrentPage=${CurrentPage}`;
    console.log("TypeConstant>>>>>>", TypeConstant);
    if (tempData) {
  ...
  fetchForEmptyState = () => {
    ...
    console.log(
      "test_user_card: " + adminGetCustomerTypesRequest.request_status
    );
  ...
  fetchForDataState = () => {
    ...
    switch (adminGetCustomerTypesRequest.request_status) {
      case customerTypeConstants.GET_CUSTOMERTYPES_SUCCESS:
        let saveRequestData =
          adminGetCustomerTypesRequest.request_data !== undefined
            ? adminGetCustomerTypesRequest.request_data.response.data
            : null;
                    ...
                    <td>
                      ...
                      >
                        <Dropdown.Item
                          eventKey="1"
                          onClick={() =>
                            this.handleShowEdit(eachCustomerype.id)
                          }
                        >
                          Edit
                        </Dropdown.Item>
  ...
  handleShowEdit = (selectedCustTypeId) =>
    this.setState({ showEdit: true, selectedCustTypeId });
  ...
  editCustomerType = () => {
    ...
    let adminUpdateCustomerTypeRequest = this.props.adminUpdateCustomerType,
      ...
      selectedCustType = adminGetCustomerTypesRequest.result.filter(
        (custType, index) => custType.id === selectedCustTypeId
      )[0];

    let allCustomerTypesData =
      this.props.adminGetCustomerTypes.request_data.response !== undefined
        ? this.props.adminGetCustomerTypes.request_data.response.data
        : null;
      ...
      return (
        ...
        >
          ...
          <Modal.Body>
            <Formik
              ...
              onSubmit={(values, { resetForm }) => {
                ...
                let custTypepayload = {
                  ...
                  clientClassification: this.state.TypeConstant,
                };

                this.handleUpdateType(custTypepayload).then(() => {
                  if (
                    this.props.adminUpdateCustomerType.request_status ===
                    customerTypeConstants.UPDATE_CUSTOMERTYPE_SUCCESS
                  ) {
                    ...
                    setTimeout(() => {
                      this.props.dispatch(
                        customerTypeActions.updateCustomerType("CLEAR")
                      );
                    }, 3000);
                  }
                });
                ...
                >
                  ...
                  <div className="footer-with-cta toleft">
                    ...
                    <Button
                      type="submit"
                      disabled={
                        adminUpdateCustomerTypeRequest.is_request_processing
                      }
                    >
                      {adminUpdateCustomerTypeRequest.is_request_processing
                        ? "Please wait..."
            ...
            {adminUpdateCustomerTypeRequest.request_status ===
              customerTypeConstants.UPDATE_CUSTOMERTYPE_SUCCESS && (
              <Alert variant="success">
                {
                  adminUpdateCustomerTypeRequest.request_data.response.data
                    .message
                }
              </Alert>
            )}
            {adminUpdateCustomerTypeRequest.request_status ===
              customerTypeConstants.UPDATE_CUSTOMERTYPE_FAILURE && (
  ...
  customerTypePopUp = () => {
    ...
    let allCustomerTypesData =
      this.props.adminGetCustomerTypes.request_data !== undefined &&
      this.props.adminGetCustomerTypes.request_data.response !== undefined
        ? this.props.adminGetCustomerTypes.request_data.response.data
        : null;
    return (
      ...
      >
        <Modal.Header>
          <Modal.Title>{this.props.TypeConstant=== TypeConstant.GROUP_CLIENT ?"Add Group Type":"Add Customer Type"}</Modal.Title>
        </Modal.Header>
        ...
        <Modal.Body>
          <Formik
            ...
            onSubmit={(values, { resetForm }) => {
              ...
              this.handleCreateNewType(custTypepayload).then(() => {
                if (
                  this.props.adminCreateCustomerType.request_status ===
                  customerTypeConstants.CREATE_CUSTOMERTYPE_SUCCESS
                ) {
                  resetForm();
                  setTimeout(() => {
                    this.retrieveFromApi();
                    this.props.dispatch(
                      customerTypeActions.addCustomerType("CLEAR")
                    );
                    this.handleClose();
                  }, 3000);
                } else {
          ...
          >
            {({
              ...
              // <Form>
              <Form noValidate className="newtype-form" onSubmit={handleSubmit}>
                <Form.Group controlId="customerTypeName">
                  <Form.Label className="block-level">Name</Form.Label>
                  <Form.Control
                    name="custTypeName"
                ...
                <div className="footer-with-cta toleft">
                  ...
                  <Button
                    type="submit"
                    disabled={
                      adminCreateCustomerTypeRequest.is_request_processing
                    }
                  >
                    {adminCreateCustomerTypeRequest.is_request_processing
                      ? "Please wait..."
          ...
          {adminCreateCustomerTypeRequest.request_status ===
            customerTypeConstants.CREATE_CUSTOMERTYPE_SUCCESS && (
            <Alert variant="success">
              {
                adminCreateCustomerTypeRequest.request_data.response.data
                  .message
              }
            </Alert>
          )}
          {adminCreateCustomerTypeRequest.request_status ===
            customerTypeConstants.CREATE_CUSTOMERTYPE_FAILURE && (
            <Alert variant="danger">
    ...
    return (
      <Fragment>
        <InnerPageContainer {...this.props}>
          ...
          {this.props.adminGetCustomerTypes.request_status ===
            customerTypeConstants.GET_CUSTOMERTYPES_SUCCESS &&
...
function mapStateToProps(state) {
  return {
    adminCreateCustomerType:
      state.administrationReducers.adminCreateCustomerTypeReducer,
    adminGetCustomerTypes:
      state.administrationReducers.adminGetCustomerTypesReducer,
    adminUpdateCustomerType:
      state.administrationReducers.adminUpdateCustomerTypeReducer,
  };
}
