...
class EditAGroup extends React.Component {
  ...
  getDetailsOfClient = ()=>{
    this.getAClient().then(()=>{
      if(this.props.getAClient.request_status === clientsConstants.GET_A_CLIENT_SUCCESS){
        let allCustomerData = this.props.getAClient.request_data.response.groupMembers;
        let memberLists = [],
          allMembersList = [];
          if(allCustomerData.length>=1){
            ...
            allCustomerData.map(eachData=>{
              ...
              groupMembers = groupMembers.filter(eachMember => eachMember.clientEncodedKey !== eachData.clientEncodedKey);
            ...
            this.setState({groupMembers:memberLists ,groupMembersData: memberLists })
          }
  ...
  getAClient = async() => {
    ...
    await dispatch(clientsActions.getAClient(this.props.match.params.encodedkey));
  };
  ...
  handleUpdateCustomer = async (updateCustomerpayload) => {
    ...
    await dispatch(clientsActions.updateAClient(updateCustomerpayload, "group"));
  };
  ...
  handleSelectedCustomer = (inputValue) => {
    ...
    if (allExistingMembers.length === 0) {
      ...
      groupMembersData = groupMembersData.filter(eachMember => eachMember.clientEncodedKey !== inputValue.clientEncodedKey);
            ...
            {({
              ...
              >
                <div className='form-heading'>
                  <h3>
                    Editing -{allCustomerData.groupName}
                  </h3>
                </div>
                <Form.Row>
                  <Col>
