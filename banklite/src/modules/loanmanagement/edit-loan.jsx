...
class EditALoanAccount extends React.Component {
    constructor(props) {
        super(props);
    ...
    loadInitialData=()=>{
        ...
        this.props.dispatch(loanActions.editALoan("CLEAR"))
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.match.params.loanId !== this.props.match.params.loanId) {
            ...
            this.loanParams.loanId = nextProps.match.params.loanId;

            if(this.loanParams.loanId !==undefined){

                this.getCustomerLoanAccountDetails(this.loanParams.loanId);
                this.getClientInfo(this.loanParams.clientId);
            }else{
                
                this.getCustomerLoanAccountDetails("CLEAR")
    ...
    editLoanAccount = async(accouuntPayload, accountType)=>{
        const {dispatch} = this.props;
        this.props.dispatch(loanActions.editALoan("CLEAR"))
        await dispatch(loanActions.editALoan(accouuntPayload, accountType));
                        ...
                        if(adminGetAllBranchesRequest.request_data.response.data.result.length>=1){
                            if(getAllUsersRequest.request_data.response.data.length>=1){
                                if(adminGetTransactionChannelsRequest.request_data.response.data.result.length>=1){
                                        ...
                                        let loanAccountValidationSchema;
                                            if(loanData.loanState===1 || loanData.loanState===2 ){
                                                loanAccountValidationSchema = Yup.object().shape({
                                            ...
                                            if(loanData.loanState===3 || loanData.loanState===5 || loanData.loanState===6 ){
                                                loanAccountValidationSchema = Yup.object().shape({
                                                    productDisplayName:  Yup.string()
                                                        .min(1, 'Valid response required')
                                                        .required('Required'),
                                            ...
                                            // console.log("account state", loanData.loanState)
                                        return (
                                            <Formik
                                                initialValues={{
                                                    ...
                                                    productDisplayName:  loanData.productname,
                                                ...
                                                onSubmit={(values, { resetForm }) => {
                                                    ...
                                                    this.editLoanAccount(loanAccountPayload, loanData.loanState)
                                                    .then(
                                                        () => {

                                                            if (this.props.editALoanReducer.request_status === loanAndDepositsConstants.EDIT_A_LOAN_SUCCESS) {
                                                                setTimeout(() => {
                                                                    // resetForm();
                                                                    // this.props.dispatch(loanActions.editALoan("CLEAR"))
                                                                }, 3000);
                                                            ...
                                                            if (this.props.editALoanReducer.request_status === loanAndDepositsConstants.EDIT_A_LOAN_FAILURE) {
                                                                setTimeout(() => {
                                                                    // resetForm();
                                                                    // this.props.dispatch(loanActions.editALoan("CLEAR"))
                                                                }, 8000);
                                                ...
                                                {({ handleSubmit,
                                                    ...
                                                    errors, }) => (
                                                        <Form
                                                            ...
                                                            <h3>Editing Loan Account ({loanData.accountNumber})</h3>
                                                            </div>
                                                            <Form.Row>
                                                        ...
                                                        {(loanData.loanState===1 || loanData.loanState===2 )&&
                                                        ...
                                                        {(loanData.loanState===1 || loanData.loanState===2 )&&
                                                            ...
                                                            <div className="footer-with-cta toleft">
                                                                ...
                                                                <Button
                                                                    type="submit"
                                                                    disabled={editALoanRequest.is_request_processing}>
                                                                        {editALoanRequest.is_request_processing?"Please wait...": "Update Loan Account"}
                                                                </Button>
                                                            </div>
                                                            {editALoanRequest.request_status === loanAndDepositsConstants.EDIT_A_LOAN_FAILURE && 
                                                                <Alert className="mt-20" variant="danger">{editALoanRequest.request_data.error} </Alert>
                                                            }

                                                            {editALoanRequest.request_status === loanAndDepositsConstants.EDIT_A_LOAN_SUCCESS && 
                                                                <Alert className="mt-20" variant="success">{editALoanRequest.request_data.response.data.message} </Alert>
                                                            }
                                                        </Form>
                                                    )}
                                            </Formik>
...
function mapStateToProps(state) {
    return {
        ...
        editALoanReducer : state.loansReducers.editALoanReducer,
...
export default connect(mapStateToProps)(EditALoanAccount);