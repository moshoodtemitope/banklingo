...
function getAClientComments(clientEncodedKey, params, tempData) {
  return (dispatch) => {
    let consume = ApiService.request(
      routes.HIT_CLIENTS +
        `/comments?ClientEncodedKey=${clientEncodedKey}&${params}`,
      "GET",
      null
    );
    dispatch(request(consume, tempData));
  ...
  function request(user, tempData) {
    ...
    if (tempData !== undefined) {
      return {
        type: clientsConstants.GET_A_CLIENT_COMMENTS_PENDING,
        user,
        tempData,
      };
    }
  ...
  function success(response) {
    return { type: clientsConstants.GET_A_CLIENT_COMMENTS_SUCCESS, response };
  }
  function failure(error) {
    return { type: clientsConstants.GET_A_CLIENT_COMMENTS_FAILURE, error };
  }
}
